...
double utils::GetVectorAngle(const Vector2f& v1, const Vector2f& v2)
{
	...
	double value = (int)(result * 100 + .5);
	double roundValue{ value / 100 };

	return roundValue;
}

double utils::GetVectorRad(const Vector2f& v1, const Vector2f& v2)
{
	//cross product
	double y = (v1.x * v2.y) - (v2.x * v1.y);
...
float utils::DotProduct(const Vector2f& v1, const Vector2f& v2)
{
	float result{ v1.x * v2.x + v1.y * v2.y };
	return result;
}

float utils::GetLength(const Vector2f& v)
{
	if (isnan(v.x))
	{
		float test();
...
float utils::CrossProduct(const Vector2f& v1, const Vector2f& v2)
{
	float cross{ v1.x * v2.y + v1.y * v2.x };
	return cross;
}

void utils::CalculateCCD(std::vector<Point2f>& points, const Point2f& target)
{
	if (points.size() < 2)
	{
		std::cout << "There must be at least 2 points declared";
		return;
	}
...
void utils::RotateAfterIndex(std::vector<Point2f>& points, const Point2f& target, const double angle, const Vector2f& rotationPoint, const size_t& index)
{
	...
	{
		RotateAroundPoint(points, target, angle, rotationPoint, i);
	}
}
